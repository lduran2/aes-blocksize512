...
function cipher(colHeight, input, w, Nb) {
	...
	const state = state.toStateArray(arr, Nb);
...
/*********************************************************************/
exports.subBytes = ((stateArray) => state.forEach(stateArray, ((el, _0, _1, _2, _3) => (sBox[el]))));

/******************************************************************//**
 * Shifts each row in the @state array cyclically
 * @params
 *   $colHeight :int = the column height, or, number of rows per column
 *   @arr :byte[] = the state array whose rows to shift
